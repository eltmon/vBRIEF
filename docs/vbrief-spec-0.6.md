...
A document is vBRIEF v0.6 conformant if and only if:

1. The root object contains a `vBRIEFInfo` field with `version` equal to `"0.6"`.
...
The normative JSON Schema for vBRIEF v0.6 is located at [`schemas/vbrief-core-0.6.schema.json`](../schemas/vbrief-core-0.6.schema.json).
